...
| Claim | Status |
|---|---|
| **"With no automatic migration path published" — an asserted ABSENCE, published on two surfaces** | **Not verified, and weaker than when it was written.** The tool's MS Project card and the marketing site both state it. An absence cannot be confirmed by reading a page that does not mention the thing, which is the exact error this register's own preamble records against §4.2. It has also moved: Microsoft's retirement guidance now points customers at **Project Server Subscription Edition** as the destination. A recommended destination is not an automatic migration path, so the claim may well still hold — but *may well* is not a source. **Decide before launch:** either narrow it to what the lifecycle record supports, or drop it. Flagged 11 Sep 2026 |
| **"Cross-project resource demand requires the enterprise-tier service" (Microsoft Project)** | **Not verified in the form we publish.** The Microsoft Learn service description for Microsoft Project was read directly (`learn.microsoft.com/office365/servicedescriptions/project-online-service-description`, ms.date 4 Sep 2023) and does not state it in those terms. What it does state is a tier gate of a different shape: *"Any interaction on a Project Online site requires at least a Project Plan 3 or Project Plan 5 subscription within the tenant."* That supports *a* tier boundary, not specifically one on cross-project resource demand. **Find the page that carries the claim, or rewrite the card to the boundary the service description actually draws.** Flagged 11 Sep 2026 |
