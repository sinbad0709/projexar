...
**16, 17 and 18 are one family and are numbered separately on purpose.** Each names a different way an assertion passes without asserting anything: 16 a stubbed API that records nothing, 17 an extraction that stops matching, 18 a generated corpus that stops rendering. The three have different tests, and a single constraint saying "make sure your assertions assert" would have caught none of them.
...
18. **A generated corpus states and floors its rendered count.** *(PR14 §4, added 11 September 2026.)* A change to the required input set can shrink a generated corpus without failing anything, so coverage that is generated rather than enumerated must assert its own size.

    This is the third member of the family §0.16 names, and the first where nothing was stubbed and nothing stopped matching. The mechanism is a correct line: a shape the form rejects is skipped, because rendering a report from an answer nobody gave would be the worse fault. It is also, unavoidably, how a new required input empties a whole section — every generated shape that does not answer the new question is skipped, the loop body never runs, and a section asserting nothing reports no failures.

    **PR13 did exactly that.** Adding the project-manager share took the seeded fuzz pass from 400 rendered shapes to 97, and not one assertion failed. The drop appeared only in the suite's own running total, which nobody reads as coverage and which moves with every commit anyway.

    **Generated, not enumerated, is the test.** A set written out shape by shape cannot shrink without someone deleting a line, and a deletion is legible in a diff. A cross product, a numeric sweep and a seeded generator have sizes produced by code and written down nowhere, so the only way a reader learns how much of one ran is if it says.

    **What it requires.** State the rendered count, and floor it. Both: a count printed and not asserted is a number in a log, and PR13's is the proof — the fuzz pass printed its total throughout and the drop from 400 to 97 went three sessions unnoticed.

    **The test.** Make a required input that most generated shapes do not answer, and watch the floor fail. Proved for each floor in PR14, isolated from the fixture sections a blanket rejection also breaks.

    Where a loop is over an enumerated set small enough to name, the stronger form is available and is preferred: fail on the skip rather than count it. PR14 converted the four that were silently skipping.
...
| Capture endpoint, PR17 §2-§5 ("PR18") | Split from PR17 at gate 1.1: blocked until the Supabase table, `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` existed as Worker secrets. **No Turnstile.** Decided on review, against the brief's own starting proposal of one widget shared with #gate's: the widget's container sits inside `#report[hidden]`, so it does not actually render until a report exists, and relocating, duplicating or repointing #gate's widget was ruled out — #gate stays exactly as it was. **Rate-limited instead**, on the native Workers `ratelimits` binding (`simple`, 20/60s, keyed on `CF-Connecting-IP`), which needs no Turnstile secret and no second widget. A `limit()` error is treated as over-limit and the request is rejected — a lost analytics row costs nothing, an endpoint with no back-stop when the binding itself errors is the actual risk. **`created_at` is not sent by the Worker.** It was originally stamped server-side into the payload; the column instead defaults to `now()` in Supabase, and the Worker no longer sends the field at all — two mechanisms producing one value would leave it unclear which was in effect, so the database owns it alone. **Column values are the raw option codes, not `selText()`'s display text**, departing from the convention that governs the printed report and the Sender payload: this store is read only by us, never by the respondent, and a code survives a copy rewrite that a label does not — `tool_version` still exists for the numeric side of that problem, and raw codes keep the categorical side out of it entirely. **Approved on review, 16 September**, on the same reasoning restated back: display text gets rewritten by copy PRs, and a relabelled option would silently split one answer into two values with nothing recording why. `claude/capacity-check-capture-queries.md`'s example queries therefore return and group on the raw codes (`msproject`, not "Microsoft Project") — expected, not a bug to "fix" if rediscovered. **`gate_passed` rows carry only `session_id`, `event` and `tool_version`** (plus the database's own `created_at` default) — no inputs, no `country`/`city` — read literally off §5's "the session identifier and the event are the whole of it". **`median_salary` is an eighteenth input**, added after gate 1.2 review: the editable ONS salary override a stored row needs to reproduce its own cost figures. **Never null, on any submit row — corrected 16 September, after shipping the opposite rule for one commit.** The first version nulled it when the field still equalled `ASHE_MEDIAN`, on the reasoning that null already means "not the respondent's own figure" elsewhere in this store. That was wrong for this column specifically: null here would mean "whatever `ASHE_MEDIAN` was at the time", and nothing maps `tool_version` to a historical ASHE figure, so those rows would not be recomputable — the one reason the column exists. It also collided with what null means everywhere else in the store, where it marks a genuinely unanswered question, and this field is never unanswered: boot pre-fills it with `ASHE_MEDIAN`, so there is no blank state to preserve, only a value, whichever source it came from. The row now always carries the effective figure the report used, asserted on both the untouched-default shape and an edited one. **Two columns carry different names in Supabase than in this document's original draft**: `tickets_per_month` (not `tickets_month`) and `run_share` (not `run_share_pct`) — the table was renamed to match the Worker, so no Worker change was needed. **`namespace_id` is `74318`, not `1001`.** Not a collision — a filesystem search found no other `ratelimits` binding in this account — but `1001` is the id in Cloudflare's own documentation example, which makes it the one anything deployed later is most likely to pick without checking first. Two bindings that share a `namespace_id` share counters, silently, with nothing in either config saying so; the id is deliberately non-default for that reason, not arbitrary |
...
| **PR14 §3: the pinned epsilon case is real but is not the general one, and a second was added.** `epsilonShape()` pins ONE call site — `roundN(x, 2)` inside `gbpBig()`, on a hero money figure, reached through a salary (£42,731.48) nobody would type. Swept both ways across 1,874 shapes, the epsilon is load-bearing at `round1()` as well and there it moves a **rated tile figure** on whole-number answers: seven BAU staff at the top of the 71–80% band is 5.6 effective FTE, which in IEEE 754 is `5.6000000000000005`, so 77 ÷ 5.60 comes out `13.749999999999998` and the tile publishes 13.7 without the epsilon where a reader dividing 77 by the 5.60 printed beside it gets 13.75 and rounds to 13.8. Pinned as `epsilonRatioShape()`; the mutation now fails at both call sites and on two different kinds of figure | Swept with and without `+ 1e-9` over 1,874 shapes, 11 September 2026 |
| **PR14 §2: four more rows printed arithmetic a reader could not redo, and one printed a sum that did not come out.** Found by generalising the BAU-tile finding into an assertion rather than by reading. All fixed in PR14. (a) **The contractor companion row** divided by the same raw effective FTE, one row below the BAU tile's, and was named nowhere. (b) **The internal effort cost** printed "10.8–13.0 × £69,251" against a product computed from £69,251.40: the loaded cost is 1.4 × salary − 750, so it carries pence for four whole-pound salaries in five, and £50,001 is an ordinary answer. (c) **Employer NI below the secondary threshold** printed "15% × (4,000 − 5,000)" beside £0, where the arithmetic shown answers −£150; the floor is the rule, so the branch now states the rule. (d) **The full portfolio cost** rounded the internal effort cost with `Math.round` while the display rounded it with `roundN`: at a salary of £50,001 the cost lands on `519,385.49999999994`, the workings printed "£519,386 + £367,000" and the total beside them read £886,385. One rounding now, the display's. Each is pinned by a named shape and each mutation fails | Each rendered and measured, 11 September 2026 |
| **PR14 §3: two tolerances in `compute()` are present, load-bearing by intent, and exercised by nothing. Listed, not fixed.** (1) `ifloor`'s `+ 1e-9`, whose comment cites `12 × 5 × (75/45)` evaluating to `100.00000000000001`; removing it leaves the suite green. (2) `redThreshold`'s `− 1e-9`, whose own comment already declares it "defensive, not a fix for anything observed"; removing it leaves the suite green. Both are **declared defensive and their absence changes nothing a reader sees**, which is what makes leaving them a diff-legibility decision rather than a judgement that they are unneeded | Mutation run on every tolerance and boundary comparison in `compute()`, 11 September 2026 |
| ~~**`CORROBORATION_TOLERANCE` could be raised from 3 to 4 with the whole suite green.**~~ | **CLOSED 11 September 2026, PR14 §1.** Listed with the two epsilons above in PR14's first report and separated from them on review, because it is a different kind of thing: they are defensive and invisible, while this sets the width of a window the report publishes, and **widening it makes the corroboration check more permissive** — the one check whose job is to catch us being wrong, weakened by one character. Nothing caught it because every shape that reaches the check sits well away from either edge: 8.A's derived run share is 71.1% to 76.0%, so the window is 68.1 to 79.0, and the existing shapes state 50, 72, 90 and 95, each of which lands the same way at any tolerance from 0 to 12. Pinned by `corroborationToleranceShapes()` — stated figures of 68, 69, 79 and 80, one point outside and one point inside each end — with the outcomes typed in from that arithmetic rather than read off either constant, so a change made in the tool and the oracle together still fails. A drift of one point in either direction now fails 12 assertions. **No behaviour change** | The mutation run at 0, 2, 4 and 12, both copies of the constant changed together, 11 September 2026 |
| **`claude/INDEX.md` was maintained by hand and asserted by nothing, and had fallen behind twice.** Two rows cited the PR11 and PR12 briefs under the `claude_` filenames that `f8437b9` renamed, so they pointed at files that did not exist. Separately, the PR13 and PR14 briefs had never been placed in the directory the index's own first rule calls the only authoritative one. **Asserted from PR14**, both ways: every `.md` file in `claude/` has an entry and every entry resolves to a file, with no document listed twice, run as part of the render suite. The PR14 brief is committed from the as-issued copy and carries a provenance marker saying so. **The PR13 brief is gone** — no copy survives outside the session it was issued in — and that is the standing example of what this assertion cannot catch: a document never placed in the directory leaves no trace in the directory or the index, so a passing run must not be read as "the specification set is complete". Two naming conventions remain in use; PR8 to PR10 still carry the `claude_` prefix and are indexed under the names they actually have | Directory and index read directly, and the assertion mutated in three directions: a rename the index did not follow, a document added without an entry, and the table format itself broken. 11 September 2026 |
| **The Panko citation supported a claim the paper contradicts, and the source had never been read.** We published *"spreadsheets carry a materially higher error rate than purpose-built systems (Panko, University of Hawaii)"* in the one place in the findings carrying an inline attribution, and in the direction that favoured our own argument. The paper concludes the opposite of its premise: *"intensive research has shown that spreadsheet error rates are comparable to those in other human cognitive activities"*, and that the cause *"is not sloppiness but rather fundamental limitations in human cognition"*. It puts complex cognitive error rates at **2% to 5%** and treats spreadsheet errors as the same phenomenon. **The difference it draws is testing, not the medium**: about a third of software development effort goes into formal testing [Grady 1994], after which errors remain in 0.1% to 0.3% of lines of code [Putnam & Myers 1992], and spreadsheets are rarely tested. What the paper DOES support, and what the page now says: field audits from 1997 onward found errors in **91% of 54** spreadsheets (Table 1; 24% weighted across all 367, but it notes the older audits used methods unlikely to catch most errors); laboratory experiments across **998 subjects and 1,170 spreadsheets** found errors in **51%** (Table 2); development experience does not reduce error rates (undergraduates, MBAs with little experience, and MBAs with 250+ hours, no significant difference); and developers are confident and wrong (median self-estimated likelihood of error 10%, mean 18%, against 86% who had made one). **A second error sat underneath the first**: `pr-sources` described a DIFFERENT paper from the one the finding cited — *What We Know About Spreadsheet Errors*, JEUC 10(2), 1998, "rev. 2008" — and its "94% of 88" belongs to the 2005/2008 revision, whose aggregation includes audits published in 2002 and 2004 and which the 1998 original therefore cannot contain. Nobody here had opened that revision. Both places now name one work, the one that was read end to end | R. Panko, *Spreadsheet Errors: What We Know. What We Think We Can Do.* Proceedings of EuSpRIG, Greenwich, 17–18 July 2000. Read in full from the open arXiv deposit, `arxiv.org/pdf/0802.3457`, not from a summary. The "94% of 88" attribution was traced to the 2005/2008 revision through secondary sources only and is **not** relied on anywhere | 11 Sep 2026 |

**This row is the reason the register has two columns.** It is the first entry recording a source that was cited for years without being opened, and the claim it carried was not merely unverified but contradicted by the paper named beside it. It was found by reading the source, not by the suite: no assertion can tell whether a sentence is supported by the work it cites. The three PR15 items that follow from it — the rewritten claim, one work named in both places, and the `pr-sources` row re-anchored to its citing sentence rather than to an answer — are in `claude/capacity-check-pr15-brief-sep-2026.md` and its addendum. **A register that records only successes teaches nothing.**
| **Project Online retires 30 September 2026.** The date the tool publishes and the marketing site repeats, read off the first-party lifecycle record rather than off the commentary, which is where it was originally taken from | Microsoft Lifecycle, `learn.microsoft.com/lifecycle/products/project-online`: *"Project Online | 3/1/2013 8:00:00 AM | 9/30/2026 8:00:00 AM"*, page updated 16 Sep 2025. Microsoft first-party, and the row the lifecycle policy is actually driven from | 11 Sep 2026 |
| **Microsoft Planner puts advanced capabilities behind a premium plan.** The source's own words are *"the advanced capabilities with Premium Plans in the new Planner"*, and the features it lists as premium are report creation, Timeline (Gantt) view, dependencies, sprints, custom fields, team workload and managing goals. **Our copy says "advanced capacity and reporting features"**: *reporting* is the source's *report creation*, and *capacity* is our word for its *team workload*. The page does not list capacity planning or resource management as premium in those terms, so the mapping is ours and is recorded here rather than implied | Microsoft Support, *Frequently asked questions about Microsoft Planner*, read directly. Microsoft first-party | 11 Sep 2026 |
| **Flexera's 67%/33% run-versus-grow split and its ~500 respondents are corroborated first-party.** Flexera's own materials give respondents' estimate of 67% run against 33% growth, and its blog describes the sample as *"more than 500 IT decision-makers"*. **The page 25 citation and the sample composition the tool publishes — N=506, worldwide, summer 2023, organisations above 2,000 employees, nearly half above 10,000 — come from the report PDF, which sits behind a registration form and has NOT been re-read here.** The headline figures stand; the provenance detail beside them is inherited | Flexera press centre and `flexera.com/blog/perspectives/navigating-the-future-of-it-investments-flexera-2023-tech-spend-pulse`. The *Flexera 2023 Tech Spend Pulse* report itself is gated | 11 Sep 2026 |
| **PR16 §4: the inherited Flexera detail above is still published today, in both places.** The N=506, page 25 and *"organisations above 2,000 employees, nearly half above 10,000"* wording sit unchanged in the on-screen and printed workings row (`Run against growth spend`, the comparison sentence beside the reported split) and in the sources row. Nobody has re-read the gated PDF since the row above was written. **Not changed here.** Proposed, not applied: either replace the inherited detail with what Flexera's own first-party press and blog materials support directly (67%/33%, *"more than 500 IT decision-makers"*), or drop the page/N/organisation-size detail and publish only the corroborated headline. This is Mark's call, because the row above already establishes the headline figures are right and only the provenance detail beside them is unread | Same sources as the row above. `public/capacity-check/index.html`, the `bauSplitEstimate` workings row and the `Run against growth spend` sources row | 15 Sep 2026 |
| **"With no automatic migration path published" was an asserted absence about a third party's product, published on the tool's MS Project card, its sources row, and three places on the marketing site.** An absence cannot be confirmed by reading a page that does not mention the thing — the error class this register's own preamble records against §4.2 — and it had weakened since it was written. **Read directly, and it names three destinations, not one:** *"Planner: A unified solution for modern work and project management"*; *"Project Server Subscription Edition: Designed for organizations needing advanced project and portfolio management (PPM) or a close match to the feature set of Project Online"*; *"Dynamics 365 Project Operations: Tailored for customers who require robust timesheet management and resource scheduling."* The page never uses the word "automatic" about migration in either direction — the only "automatic" in it describes the retirement date itself, *"this retirement will happen automatically on the specified dates with no admin action required"*, which is a different claim from a migration path and was never the one published. The tool's MS Project card and sources row now name the three destinations instead of asserting an absence | George Bullock, Microsoft, *Microsoft Project Online is retiring: What you need to know*, Planner Blog, Microsoft Community Hub, published 5 Sep 2025, `techcommunity.microsoft.com/blog/plannerblog/microsoft-project-online-is-retiring-what-you-need-to-know/4450558` — the retirement guidance linked from the Microsoft Lifecycle page for Project Online. Microsoft first-party | 15 Sep 2026 |
| **"Cross-project resource demand requires the enterprise-tier service" (Microsoft Project) was published on the tool's MS Project card and its sources row, and the Microsoft Learn service description does not state it in those terms.** Read directly again — `learn.microsoft.com/en-us/office365/servicedescriptions/project-online-service-description/project-online-service-description`, ms.date 4 Sep 2023, unrevised since the PR15 read: *"Any interaction on a Project Online site requires at least a Project Plan 3 or Project Plan 5 subscription within the tenant."* That is a per-seat licence floor on interacting with a Project Online site at all, not a feature gate on cross-project resource demand specifically, and no other Microsoft page carrying the claim in the published form was found. The card and sources row now state the licence floor the source actually draws | Microsoft Learn, *Microsoft Project service description*, read directly. Microsoft first-party | 15 Sep 2026 |

**Not verified — do not publish, and do not decide on, without checking first**

| Claim | Status |
|---|---|
| ~~"With no automatic migration path published" — an asserted ABSENCE, published on two surfaces~~ | **RESOLVED 15 Sep 2026, PR16.** See the verified entry above: Microsoft's retirement guidance names three transition destinations and never uses "automatic" about migration itself. The tool's MS Project card and sources row are corrected. The marketing site's occurrences (three files, seven strings) are reported in the PR16 handover and left unchanged — Mark's call, not this PR's |
| ~~"Cross-project resource demand requires the enterprise-tier service" (Microsoft Project)~~ | **RESOLVED 15 Sep 2026, PR16.** See the verified entry above: the service description states a per-seat licence floor on interacting with a Project Online site, not a gate specifically on cross-project resource demand. The tool's MS Project card and sources row now state the licence floor |
...
| ~~**The corroboration workings row states its two outcomes the wrong way round.**~~ | **CLOSED 11 September 2026, PR14 §1.** The cell printed the derived window and the respondent's figure, then labelled a figure ABOVE the window "Below the window" and one below it "Above the window, stated not rated", on every shape, from PR1 until now. The check, its branches and the card copy were right throughout, which is what made it invisible: nothing in a 46,085-assertion suite read the row against the comparison three characters to its left. **The two labels are swapped and nothing else.** Asserted derivationally — the direction word is read against the window and the stated figure the same cell prints — on 642 rendered rows, with all three branches asserted reached (84 above, 9 below, 549 inside) and the card copy asserted to agree with the row on each. Re-inverting the labels fails 186 assertions. **Cost:** the two labels, one inverted code comment beside the card copy that had said "above" since PR7 and would have reintroduced it, and one new section |
| ~~**The BAU tile's published ratio divides by the raw effective FTE, not the displayed one.**~~ | **CLOSED 11 September 2026, PR14 §2.** Resolved by printing the divisor at the precision the division used, not by moving the ratio: `bau_effective_fte` is a headcount times a whole-percent band endpoint, so it is exact to two places, and the workings row now prints "45 ÷ 4.96" where it printed "45 ÷ 5.0". **The rated figure is unchanged and is asserted unchanged** — `projectsPerFTE` still divides by the unrounded quantity, which §2.6 ties the growth ceiling's divisor to and PR13 §5 forbids moving. Generalised into an assertion that every printed calculation in the workings reproduces from its own printed operands, at the precision the row published, across 15,883 calculations on 689 shapes, with every one of the table's 33 row labels asserted reached and any label the table does not account for failing. **That assertion found four more rows of the same class**, all listed below and all fixed in the same change |
...
| The Wrangler version this repo deploys with is pinned | **It is not, and that is an open item, not a fact anyone has checked and found true.** There is no `package.json` and no CI in this repo, so nothing pins the toolchain. `npx wrangler --version` resolved to 4.120.0 in the session that added the PR17 §2-§5 `ratelimits` binding (16 Sep 2026), comfortably past the 4.36.0 floor that binding needs, but a deploy run from a different machine is not guaranteed to resolve the same version, or one recent enough. Not blocking this PR. **Gets its own PR after this one** |
