...
| **`report_consent` has no Sender custom field, and the Worker did not enforce consent.** The payload has carried the key since 9 Aug and Sender discards it on arrival; separately, `handleCapacityReport` recorded the value and subscribed either way. The enforcement is fixed in PR11; **creating the field is an account action and is still outstanding** | Sender audit of 10 Sep 2026, field table row 14: absent from the group export and from every activity log | 10 Sep 2026 |
