...
**Constraints 11 to 15 were added by PR briefs and never folded in here, which is why this list stopped at 10 while the code and the suite cited §0.11, §0.14 and §0.15 in twenty-six places.** They are transcribed below verbatim from the briefs that declared them, with no wording changed. Folded in 10 September 2026 (PR10), because 16 could not be added to a list with a hole in it.
...
15. **No change to the Sender field contract in PR9.** No field added, removed or renamed, and no value's meaning changed. *(PR9 brief §0.* Its stated reason — that the reconciliation waits for PR10 "because PR10 moves the internal cost figure" — **did not come to pass.** PR10's numeric invariant was absolute and no figure moved on any of the 663 shapes the suite renders, so nothing in this PR re-audits a Sender field. The deferral now rests on nothing; reschedule it on its own merits.*)
